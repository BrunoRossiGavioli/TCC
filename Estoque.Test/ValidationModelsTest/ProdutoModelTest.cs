...
namespace Estoque.Test.ValidationModelsTest
{
    public class ProdutoModelTest
    {
        ...
        #region Fornecedorid
        [Fact(DisplayName ="Fornecedor não pode ser nulo")]
        public async Task FornecedorNulo()
        {
            var instance = _builder.With(x => x.FornecedorId = 0).Build();
            var validation =await _validator.ValidateAsync(instance);
            Assert.False(validation.IsValid);
            Assert.Contains(validation.Errors, x=>x.ErrorMessage.Contains(MensagensErroProduto.FornecedorVazio));
        }
        #endregion
    }
}
